...
contains
    ...
    subroutine sub_modelSetStates(capital_grid,shocks_vector,transitionFunction)
        ...
        IF (size(capital_grid,dim=1)/=n) THEN
            ...
            call sub_mystop('program terminated by sub_modelSetStates')
        END IF
        ...
        integer :: index_k, index_z
            ...
            do index_k = 1,length_grid_k
                do index_z = 1,n_tauchen
                    y(index_k,index_z)=beta*dot_product(shocks(index_z,:),Vend1(index_k,:))
                enddo
            enddo
...
contains
    ...
    subroutine sub_endogenize(fnToSolve, grid_k, y, transition)
        ...
        do while((diff>toler) .and. (iter<MAX_ITER))
            ...
            call getNextGuess(fnToSolve,Cstar,Yend,cih)

            do index_z = 1,n
                call sub_myinterp1(Yend(:,index_z),Cstar(:,index_z), cih, length_grid_k, Vend1(:,index_z))
            enddo
...
program main
    ...
    call sub_endogenize(valuefn,g_k,g_c,grid_k,y,transition)
